	...
	*/
			...
			*/
				...
				update: function (el, cb) {
					var selector =
						el || _selector.call(this) /* validate selector */
						...
						if ($this.data(pluginPfx)) {
							...
							if (cb && d) {
								if (
									...
									typeof o.callbacks.onImageLoad ===
										'function'
								) {
				...
				destroy: function () {
					...
					return $(selector).each(function () {
						...
						if ($this.data(pluginPfx)) {
							...
							if (o.live) {
								removeLiveTimers(
									o.liveSelector || $(selector).selector,
								)
							} /* remove live timers */
					...
					*/
						mCSB_container
							...
							*/
								width:
									Math.ceil(
										mCSB_container[0].getBoundingClientRect()
											.right + 0.4,
									) -
									Math.floor(
										mCSB_container[0].getBoundingClientRect()
		...
		*/
			_draggable = function () {
				...
				function _drag(dragY, dragX, y, x) {
					mCSB_container[0].idleTimer =
						o.scrollInertia < 233 ? 250 : 0
					if (draggable.attr('id') === draggerId[1]) {
						var dir = 'x',
							to =
								(draggable[0].offsetLeft - dragX + x) *
								d.scrollRatio.x
					} else {
						var dir = 'y',
							to =
								(draggable[0].offsetTop - dragY + y) *
								d.scrollRatio.y
					}
			...
			_contentDraggable = function () {
				...
				function _onTouchmove(e) {
					...
					if (draggable) {
						...
						touchMoveY.push(y)
						touchMoveX.push(x)
						touchIntent[2] = Math.abs(
							_coordinates(e)[0] - touchIntent[0],
						)
						touchIntent[3] = Math.abs(
							_coordinates(e)[1] - touchIntent[1],
						)
						if (d.overflowed[0]) {
							var limit =
									mCSB_dragger[0].parent().height() -
		...
		*/
			_selectable = function () {
				...
				mCSB_container
					...
					.bind(
						'mouseup.' + namespace + ' dragend.' + namespace,
						function (e) {
							if (touchable) {
								return
							}
				...
				mCSB_draggerContainer
					...
					.bind('click.' + namespace, function (e) {
						...
						if (
							...
							if (
								el.parent('.mCSB_scrollTools_horizontal')
									.length > 0
							) {
								if (!d.overflowed[1]) {
									return
								...
								var dir = 'x',
									clickDir =
										e.pageX > mCSB_dragger.offset().left
											? -1
											: 1,
									to =
										Math.abs(mCSB_container[0].offsetLeft) -
								...
								var dir = 'y',
									clickDir =
										e.pageY > mCSB_dragger.offset().top
											? -1
											: 1,
									to =
										Math.abs(mCSB_container[0].offsetTop) -
			...
			_focus = function () {
				...
				mCSB_container.bind('focusin.' + namespace, function (e) {
					var el = $(document.activeElement),
						nested =
							mCSB_container.find('.mCustomScrollBox').length,
						dur = 0
				...
				function _on(once) {
					...
					var c = seq.type !== 'stepped' /* continuous scrolling */,
						...
						px =
							seq.dir[0] === 'x'
								? contentPos[1] +
								  seq.dir[1] * parseInt(seq.scrollAmount)
								: contentPos[0] +
								  seq.dir[1] * parseInt(seq.scrollAmount),
				...
				switch (t) {
					...
					case 'number':
						if (_isNumeric(val)) {
							...
							/* percentage value */
							return Math.abs(
								(contentLength * parseInt(val)) / 100,
							)
						} else if (val.indexOf('-=') !== -1) {
							/* decrease value */
							return Math.abs(
								contentPos - parseInt(val.split('-=')[1]),
							)
						} else if (val.indexOf('+=') !== -1) {
							/* inrease value */
				...
				function upd() {
					...
					mCSB_container[0].autoUpdate = setTimeout(function () {
						...
						if (o.advanced.updateOnImageLoad) {
							if (
								!(
									o.advanced.updateOnImageLoad === 'auto' &&
									o.axis === 'y'
								)
							) {
			...
			_scrollTo = function (el, to, options) {
				...
				switch (options.dir) {
					case 'x':
						var mCSB_dragger = $(
							...
							totalScrollOffset =
								tso > 0 ? tso / d.scrollRatio.x : 0,
							totalScrollBackOffset =
								tsbo > 0 ? tsbo / d.scrollRatio.x : 0
						break
					case 'y':
						var mCSB_dragger = $(
								'#mCSB_' + d.idx + '_dragger_vertical',
							...
							totalScrollOffset =
								tso > 0 ? tso / d.scrollRatio.y : 0,
							totalScrollBackOffset =
								tsbo > 0 ? tsbo / d.scrollRatio.y : 0
						break
				}
				if (
					scrollTo[1] < 0 ||
					(scrollTo[0] === 0 && scrollTo[1] === 0)
				) {
					scrollTo = [0, 0]
					...
					options.overwrite,
					{
						...
						onComplete: function () {
							if (options.callbacks && options.onComplete) {
								if (o.axis === 'yx') {
									clearTimeout(
										mCSB_container[0].onCompleteTimeout,
									)
								}
								...
								mCSB_container[0].onCompleteTimeout =
									setTimeout(function () {
										...
										if (
											_cb('onTotalScroll') &&
											scrollTo[1] >=
												limit[1] - totalScrollOffset &&
											d.cbOffsets[0]
										) {
											_mcs()
											o.callbacks.onTotalScroll.call(
												el[0],
											)
										}
										if (
											...
											_mcs()
											o.callbacks.onTotalScrollBack.call(
												el[0],
											)
										}
										d.tweenRunning = false
				...
				function _cb(cb) {
					return (
						d &&
						o.callbacks[cb] &&
						typeof o.callbacks[cb] === 'function'
					)
				}
				/* checks whether callback offsets always trigger */
				function _cbOffsets() {
					return [
						o.callbacks.alwaysTriggerOffsets ||
							contentPos >= limit[0] + tso,
			...
			$.extend($.expr[':'], {
				...
				mcsInSight:
					...
					function (el, i, m) {
						...
						return (
							pos[0] - wrapperView[0] * wrapperViewPct[0][0] <
								0 &&
							pos[0] +
								elD[0] -
								wrapperView[0] * wrapperViewPct[0][1] >=
								0 &&
							pos[1] - wrapperView[1] * wrapperViewPct[1][0] <
								0 &&
							pos[1] +
								elD[1] -
								wrapperView[1] * wrapperViewPct[1][1] >=
								0
						)
					},
