		...
		<div id="sub_wrap">
			<div class="title">
				<ul>
					...
					<li>
						<i class="fa fa-chevron-right"></i> 오픈소스 라이브러리
					</li>
					<li><i class="fa fa-chevron-right"></i> 반입 신청</li>
				</ul>
				<h3>오픈소스 라이브러리 반입 신청</h3>
			</div>
			<div class="subcontents">
				<div class="s_title border_no">
					<div class="page_tit">
						<h4>반입 신청 정보입력</h4>
						<span>
							오픈소스 반입 신청은 프로젝트 단위로 할 수 있으며,
							반입완료 후 프로젝트 분석환경에서 설치 및 활용이
							가능합니다.
						</span>
					</div>
				</div>
											...
											>
												...
												<v-text-field
													v-if="
														...
														'etc'
													"
													placeholder="패키지 그룹명을 입력해 주세요"
													outlined
													clearable
													v-model="etcPackageName"
...
export default {
	data() {
		return {
			...
			libname: '',
			libnameRules: [v => !!v || '라이브러리명을 입력해 주세요'],
			libver: '',
			libverRules: [v => !!v || '라이브러리 버전을 입력해 주세요'],
			libfilename: '',
			libfilenameRules: [v => !!v || '라이브러리 파일명을 입력해 주세요'],
			reason: '',
			reasonRules: [v => !!v || '반입 신청 사유를 입력해 주세요'],
		}
	},
	created() {
		this.setProjectList()
	...
	methods: {
		...
		setPackageName() {
			if (this.radioPackageName === 'etc') {
				...
				this.packageName = this.radioPackageName
			}
		},
		submit() {
			...
			if (this.$refs.form.validate()) {
				...
				console.log('package group : ' + this.packageName)
				console.log('libname : ' + this.libname)
				console.log('libver : ' + this.libver)
				console.log('libfilename : ' + this.libfilename)
